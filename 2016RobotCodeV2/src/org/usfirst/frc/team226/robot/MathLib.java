...
public class MathLib {
	 ...
	 * forms a parabola starting and ending at minSpeed when you start and end
	 * and achieving maxSpeed exactly halfway.
	 *
	 * @param minSpeed
	 ...
	 *         Note that since velocity is a parabola, acceleration is linear.
	 *         It may exceed the maximum value robot can accelerate without
	 *         wheel slipping.
	 */
	public static double getSpeedForLinearAccel(double minSpeed, double maxSpeed, double percentComplete) {
		return 4 * (minSpeed - maxSpeed) * (percentComplete - 0.5) * (percentComplete - 0.5) + maxSpeed;
	}
	 ...
	 */
	public static double getSpeedForConstantAccel(double minSpeed, double maxSpeed, double percentComplete) {
		return maxSpeed + 2 * (minSpeed - maxSpeed) * Math.abs(percentComplete - 0.5);
	}

	/***
	 * Restricts an input value to the range [low, high]. If value > high it
	 * will be set to high. If value < low it will be set to low.
	 *
